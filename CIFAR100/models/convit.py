...
class Block(nn.Module):
    ...
    def forward(self, x, mask_heads=None, task_index=1, attn_mask=None, moe=False, return_gates=False, task=None):
        if isinstance(self.attn, ClassAttention) or isinstance(self.attn, JointCA):  # Like in CaiT
            ...
            if moe:
                y, *aux = self.mlp(self.norm2(cls_token), return_gates=return_gates, task=task)
                cls_token = self.drop_path(y) + cls_token
                return cls_token, aux
        ...
        if moe:
            y, *aux = self.mlp(self.norm2(x), return_gates=return_gates, task=task)
...
class ConVit(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        print("patch size: ", patch_size, "img_size", img_size)
    ...
    #         elif name == 'old_heads':
    #             self.classifier.freeze(name)
    #         elif name == 'backbone':
    #             freeze_parameters(self.blocks)
    #             freeze_parameters(self.patch_embed)
    #             freeze_parameters(self.pos_embed)
    ...
    def forward_features(self, x, final_norm=False, distillation=False):
        B = x.shape[0]
        x = self.patch_embed(x)
...
class MoEConVit(ConVit):
    def __init__(self, moe_args, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        super().__init__(img_size, patch_size, in_chans, num_classes, embed_dim, depth,
                 ...
                 local_up_to_layer, locality_strength, use_pos_embed,
                 class_attention, ca_type)
    ...
    def _build_moe_blocks(self, moe_args):
        ...
        upcycle_order = moe_args["upcycle_order"]
        self.n_upcycle = int(len(self.blocks) * upcycle_ratio)
        self.moe_indices = []
        for i, block in enumerate(self.blocks):
            conditions = [
                i < self.n_upcycle and upcycle_order == "sparse_first",
                i >= len(self.blocks) - self.n_upcycle - 1 and i < len(self.blocks)
                and upcycle_order == "dense_first",
                upcycle_order == "interleave" and (i-1) % int(1. / upcycle_ratio) == 0 and i > 0,
            ]
            if any(conditions):
                print("Adding MoE layer to block ", i)
                block.mlp = SMoE(**moe_args, state_dict=block.mlp.state_dict())
                self.moe_indices.append(i)

    def forward_features(self, x, final_norm=False, distillation=False, task=None):
        ##################
        losses = 0
        dropped_tokens = 0
        loads = []
        ...
        for i, blk in enumerate(self.blocks[:self.local_up_to_layer]):
            if blk.mlp.__class__.__name__ == "Mlp":
                x, _, _ = blk(x)
            else:  # MoE block
                x, (loss, importance, load, _gates, aux_loss, dropped) = blk(
                    x, moe=True, return_gates=distillation, task=task
                )
                losses += loss
                losses += aux_loss
        ...
        if self.use_class_attention:
            for i, blk in enumerate(self.blocks[self.local_up_to_layer:]):
                if blk.mlp.__class__.__name__ == "Mlp":
                    ...
                    cls_tokens, (loss, importance, load, _gates, aux_loss, dropped) = blk(
                        torch.cat((cls_tokens, x), dim=1), moe=True, return_gates=distillation, task=task
                    )
                    losses += loss
                    losses += aux_loss
                    dropped_tokens += dropped
        ...
        loads = torch.stack(loads, dim=0).float()

        loads /= loads.sum(dim=1, keepdim=True).float() # normalize to percentages

        if self.use_class_attention:
            return cls_tokens[:, 0], losses, loads, dropped_tokens
        else:
            return x[:, 0], losses, loads, dropped_tokens
...
def construct_moe_convit(num_classes):
    moe_args = dict(
        num_experts=4,
        k=1,
        noisy_gating=False,
